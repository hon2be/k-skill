...
- `list.json`의 공식 요청 파라미터에는 `corp_name` 이 존재하지 않음. 회사명 기준으로 좁히려면 `corpCode.xml` ZIP을 받아 `corp_code`(8자리 고유번호)를 먼저 확보한 뒤 `corp_code` 로 호출. `corp_code` 자체는 선택사항(공식 가이드: N)이지만, 외부 사용 사례상 미지정 시 검색 기간이 3개월 이내로 제한되는 경향이 관찰됨.
